...
public class USLocalizer {
	...
	private static final float WALL_DISTANCE = 25;
	private static final float EDGE_DISTANCE = (float) 10.4;
	private static final float EDGE_DISTANCE_R = 20;
		...
		if (locType == LocalizationType.FALLING_EDGE) {
			...
			while (true) {
				if (!noiseZone && getFilteredData()<EDGE_DISTANCE+MARGIN_DISTANCE) {
					...
					noiseZone = true;
					Sound.beep();
				} else if ( noiseZone && getFilteredData()<EDGE_DISTANCE-MARGIN_DISTANCE){
			...
			while (true) {
				...
				if ( getFilteredData()>EDGE_DISTANCE_R+MARGIN_DISTANCE){
					angleB = (angleB + odo.getAng())/2;
					noiseZone = false;
					Sound.beep();
					break;
